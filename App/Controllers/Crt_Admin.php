...
class Crt_Admin extends \Core\Controller
{
    public static function infosuser()
    {
        ...
        $table_ane="personne"; 
        $tb_conditions_ane=[];
        $tb_conditions_ane["id_pers_personne"]=$tabdata_user->id_pers_personne;
        $tmp_info_user = Admin::get_selectSQL_ALL_by($table_ane, $tb_conditions_ane);
        //var_dump($tmp_info_user);
    ...
    public function accueilAction()
    {
        ...
        if ( $get_user_conex ==0) {
            ...
            $tabdatas['eleve_conect'] = Admin::get_useronline_by(1,$tabdata_user->fk_iduniv);
            $tabdatas['prof_conect'] = Admin::get_useronline_by(2,$tabdata_user->fk_iduniv);
            $tabdatas['parent_conect'] = Admin::get_useronline_by(3,$tabdata_user->fk_iduniv);
            $tabdatas['admin_conect'] = Admin::get_useronline_by(4,$tabdata_user->fk_iduniv);
    ...
    public function admin_comptabiliteAction()
    {
        ...
        if (isset($_POST['nomeleve']) && isset($_POST['code_recu']) && isset($_POST['btn_suppaiement'])) {
            //var_dump($_POST);

            $cp_ideleve = intval(htmlspecialchars($_POST['ideleve']));
            $cp_ideleve_pers = htmlspecialchars($_POST['ideleve_pers']);
            $cp_montant = intval(htmlspecialchars($_POST['montant']));
            $cp_code_recu = htmlspecialchars($_POST['code_recu']);
            $cp_nomeleve = htmlspecialchars($_POST['nomeleve']);

            $table="versement";
            $tb_conditions=[];
            $tb_conditions["id_eleve"]= $cp_ideleve;
            $tb_conditions["ligneversement"]= $cp_code_recu;
            $delete_vers = Admin::set_deleteSQL_ALL_by($table,$tb_conditions);

            $table="operation";
            $tb_conditions=[];
            $tb_conditions["id_pers"]= $cp_ideleve;
            $tb_conditions["code_versement"]= $cp_code_recu;
            $delete_ope = Admin::set_deleteSQL_ALL_by($table,$tb_conditions);
            
            $table="partdev";
            $tb_conditions=[];
            $tb_conditions["id_eleve"]= $cp_ideleve;
            $tb_conditions["code_ligneversement"]= $cp_code_recu;
            $delete_pdev = Admin::set_deleteSQL_ALL_by($table,$tb_conditions);

            $table="ligneversement";
            $tb_conditions=[];
            $tb_conditions["id_eleve"]= $cp_ideleve;
            $tb_conditions["code_ligneversement"]= $cp_code_recu;
            $delete_ligvers = Admin::set_deleteSQL_ALL_by($table,$tb_conditions);            

            $fct_exec = "versement :".$delete_vers."; operation: ".$delete_ope." ; partdev =".$delete_pdev."; ligneversement=".$delete_ligvers;


            $fct_exec = $fct_exec ." |INFOS| Étudiant : ".$cp_nomeleve."(".$cp_ideleve.";". $cp_ideleve_pers.") | recu :".$cp_code_recu." | Montant : ".$cp_montant;

            /*:::::::DEBUT Enregistrement des logs::::::::::*/
            $info = "Crt_Admin ::: admin_comptabiliteAction => " . $fct_exec;
            $log_user =" Suppression de paiement ";
            modeldb::set_Ajax_Log($info,$log_user,$id_pers,$fk_iduniv);
            //:::::::::::::LOGS::::::::::::::::::
            /*:::::::Fin Enregistrement des logs::::::::::*/
             $fct_exec = "||";
        ...
        if ($all_versem!= 0 && !empty($all_versem)) {
        ...
        $list_class=[];
        $tb_conditions=[];
        $tb_conditions['groupe_annee']=$tabdatas['lib_user_id_annee'];
        $list_class = Admin::get_selectSQL_ALL_by($table, $tb_conditions);
        if (!empty($list_class) && $list_class!=0) {
            $tabdatas['list_class']=$list_class;
        }

        //var_dump($tabdatas['list_class'][0]);
        //var_dump($_POST);

        if( isset($_POST['moy_classe']) && isset($_POST['btn_afficheMoy'])){
            ...
            $list_class=[];
            $tb_conditions=[];
            $tb_conditions['groupe_id']=$id_grpe;
            $tb_conditions['groupe_annee']=$tabdatas['lib_user_id_annee'];
            $tabdatas['infos_groupe'] = Admin::get_selectSQL_ALL_by($table, $tb_conditions);
            if (!empty($tabdatas['infos_groupe']) && $tabdatas['infos_groupe']!=0) {
                $tabdatas['infos_groupe'] = $tabdatas['infos_groupe'][0];
            }
            //var_dump("get_univtype_scolarite", $tabdatas['get_univtype_scolarite']);
            $tabdatas['infos_groupe']['scolarite_O_AFF'] = 0;
            $tabdatas['infos_groupe']['scolarite_N_AFF'] = 0;
            
            foreach ($tabdatas['get_univtype_scolarite'] as $key => $value) {
               if ($value["niveau"]==($tabdatas['infos_groupe'])['fk_idniveau'] && $value["classe"]==($tabdatas['infos_groupe'])['groupe_classe']) {
                   
                   if ($value["status_affecte"]=='O_AFF') {
                       $tabdatas['infos_groupe']['scolarite_O_AFF']=$tabdatas['infos_groupe']['scolarite_O_AFF'] + $value["montant"];
                   } else{
                       $tabdatas['infos_groupe']['scolarite_N_AFF'] = $tabdatas['infos_groupe']['scolarite_N_AFF'] + $value["montant"];
                   }

               }
            }

            //var_dump("infos_groupe",$tabdatas['infos_groupe']);
            $getAllElevByGrp = Admin::getAllElevByGrp($id_grpe);
            $tabdatas['getAllElevByGrp']= $getAllElevByGrp;
           
            foreach ($getAllElevByGrp as $key => $value) {
                $table='ligneversement';
                $list_class=[];
                $tb_conditions=[];
                $tb_conditions['id_eleve']=$value['id_eleve_eleve'];
                $ligneversement = Admin::get_selectSQL_ALL_by($table, $tb_conditions);
                ($tabdatas['getAllElevByGrp'][$key])['ligneversement']= $ligneversement;


            }
            //var_dump($tabdatas['getAllElevByGrp'][0]);
            //var_dump($tabdatas['get_univ_scolarite']);
        ...
        $id_annee= $tabdata_user->lib_user_id_annee;
        $annscolaire= explode(" ", $tabdata_user->lib_user_annee);
        //var_dump($annscolaire);exit;
        $all_etudinfos=Admin::get_all_univ_etudiant($annscolaire[0], $id_annee , $id_univ);

       
        foreach ($all_etudinfos as $key => $value) {
            $table='ligneversement';
            $list_class=[];
            $tb_conditions=[];
            $tb_conditions['id_eleve']=$value['id_eleve'];
            $ligneversement = Admin::get_selectSQL_ALL_by($table, $tb_conditions);
            $tabdatas['all_etudinfos'][$key] =$value ;
            ($tabdatas['all_etudinfos'][$key])['ligneversement'] = $ligneversement;
        }


        //$tabdatas['all_etudinfos'] =$all_etudinfos;
        //var_dump($tabdatas['all_etudinfos'][0]);


        //var_dump($tabdatas['tab_pane']);
        /*:::::::DEBUT Enregistrement des logs::::::::::*/
        $info = "Crt_Admin ::: admin_comptabiliteAction => " . $fct_exec;
            ...
            foreach ($annee_partie as $key_part => $val_part) {
                ...
                foreach ($tabdatas['get_info_onMoy'] as $key => $value) {    
                    ...
                    if ($value['fk_part_annee'] == $val_part['id_annee_partie'] && !(in_array($test_idelep,$tb_idelev))) {
                        ...
                        $tmp_flot_moy =floatval($value['moyenne']);
                        if ($tmp_flot_moy >=0 && 20>= $tmp_flot_moy) {
                ...
                $tb_conditions["fk_id_grpe"]=$moy_grpe;
                $tmp_renit_rang = Admin::set_updateSQL_ALL_by($table,$tb_infos, $tb_conditions);

                $tmp_moy = (float)0;
                $tmp_rang=0;
                $tmp_rang_verif=0;
                //var_dump($tb_moy);exit();
                //Parcours de la liste des moyenne du plus grand au plus petit
                $table = 'moyenne_matf';         
                ...
                foreach ($tb_moy as $key => $value) {
                    ...
                    if ($tmp_moy != $value) {
                        ...
                        $tb_infos['rang_mat']=$tmp_rang_verif;
                        //$test_moymat = Admin::set_updateSQL_ALL_by($table,$tb_infos, $tb_conditions);
                        $test_moymat = Admin::set_updateSQL_Moyenne_by($moy_mat,intval($val_part['id_annee_partie']), $moy_grpe,$value,$tmp_rang_verif);
                        //var_dump("Admin::set_updateSQL_Moyenne_by(".$moy_mat.",".intval($val_part['id_annee_partie']).", ".$moy_grpe.",".$value.",".$tmp_rang_verif.")");
                        
                        
                        //var_dump($tmp_rang_verif.'  result='. $test_moymat.' moy='.$value);
                        //var_dump("MOY DIFF Rang :".$tmp_rang_verif.'  result='. $test_moymat.' moy='.$value.' !=tmp_moy='.$tmp_moy);
                        $tmp_moy = $value;
                    }
                    else{
                        $test_moymat=0; 
                        //var_dump("MOY EGALE Rang :".$tmp_rang_verif.'  result='. $test_moymat.' moy='.$value.' !=tmp_moy='.$tmp_moy);
                    }
    ...
    public function prof_infosAction()
    {
        ...
        //var_dump($tabdatas['prof_id_pers']);
        /*
    ...
    public function elev_listAllAction()
    {
        ...
        if (isset($_POST['btn_voir_eleve']) && $_POST['id_eleve'] != "" && $_POST['id_eleve'] != null) {
            ...
            if (isset($tabdatas['get_elevBy']) && !empty($tabdatas['get_elevBy'])) {
    ...
    public function admin_inscrip_etudAction()
    {
        ...
        $_POST['anneeScolaire_stage']=(explode(" ",$tabdatas['lib_user_annee']))[0];
        $_POST['id_anneescol']=$tabdatas['lib_user_id_annee'];

        $_POST['btn_voir_dmd']="";
        //var_dump($_POST);
        $table="preinscription";
        $tb_conditions=[];
        $tb_conditions["annee_scola"]=$_POST['anneeScolaire_stage'];
        $liste_inscription= Admin::get_selectSQL_ALL_by($table, $tb_conditions);

        if(!empty($liste_inscription) && $liste_inscription!=0){
            foreach ($liste_inscription as $key => $value) {
                $tmp_info_elevegrp= Admin::get_elev_allgrp_Byannee($value['id_eleve'],$tabdatas['lib_user_id_annee']);
                //var_dump($tmp_info_elevegrp);
                if ($tmp_info_elevegrp!=0) {
                    $table="preinscription";
                    $tb_infos=[]; 
                    $tb_conditions=[];
                    $tb_conditions["annee_scola"]=$_POST['anneeScolaire_stage'];
                    $tb_conditions["id_eleve"]=$value['id_eleve'];
                    $tb_infos["niveau"]=intval($tmp_info_elevegrp[0]['fk_idniveau']);
                    $tb_infos["classe"]=intval($tmp_info_elevegrp[0]['groupe_classe']);
                    $liste_inscription_update= Admin::set_updateSQL_ALL_by($table,$tb_infos, $tb_conditions);
                }            
            }
        }
        ...
        if ( isset($_POST['id_etudiant']) && isset($_POST['id_pers_etudiant']) && isset($_POST['matricule']) && isset($_POST['filiere_id']) && isset($_POST['niveau_id']) && isset($_POST['statut_affect']) && isset($_POST['statut_boursier']) && isset($_POST['statut_redoub']) && isset($_POST['btn_activeinscrip']) ) {

            //var_dump($_POST,$tabdatas['initiale_univ']);exit;
            ...
            //var_dump( $tabdatas['initiale_univ']);


            if ($tabdatas['set_MajPreinscriptionBy'] == 1 || $tabdatas['set_MajTble_eleveBy'] == 1) {
                if (isset($tabdatas['initiale_univ']) && $tabdatas['initiale_univ'] == 'aibs') {
                    $tabdatas['set_MajEtat_persBy'] = Admin::set_MajEtat_persBy($id_pers_etudiant, 3);
                }
                else {
            ...
            if (!empty($list_part_annee ) && !empty($tabdatas['get_gpreEtudiantsBy'])) {
                foreach ($list_part_annee as $cle => $val) {
                    ...
                    foreach ($tabdatas['get_gpreEtudiantsBy'] as $key => $value) {
                        ...
                        //var_dump('get_gpreEtudiantsBy',$tabdatas['get_gpreEtudiantsBy']);exit();
                       
                        //if( $value['id_eleve_eleve']==89) { var_dump('get_eleve_all_moy',$tabdatas['get_eleve_all_moy']); }
                        ...
                        //if ( $value['id_eleve_eleve']==89) { var_dump($tb_mat1); }
                        if ($tb_mat1!=0) {
                            foreach ($tb_mat1 as $key => $valuemat) {
                                $var_test=0;
                                if ( intval($valuemat['id_annee_partie'])== $part_anneid ) {
                                    $moyenne_tmp = Admin::get_elevmoy_by($id_eleve,intval($valuemat['matiere_id_tmp']),$tabdatas['id_groupe'],$part_anneid);
                                    //var_dump($moyenne_tmp);var_dump($value);exit;
                                    //var_dump('valuemat',$valuemat);
                                    //var_dump('moyenne_tmp',$moyenne_tmp);
                                    if ($moyenne_tmp['moyenne']==null ) {
                                        $moyenne_tmp['moyenne']=0;
                                    }
                                    if (intval($moyenne_tmp['moyenne'])>20 ||  0>intval($moyenne_tmp['moyenne'])) {
                                        $moyenne_tmp['moyenne']=-10;
                                    }

                                    $tb_mat2[$key]=$valuemat;
                                    $tb_mat2[$key]['moyenne']=floatval($moyenne_tmp['moyenne']);

                                    if ($valuemat['coeficient_tmp'] != 0 && $valuemat['coeficient_tmp'] != '0' && 20>=intval($moyenne_tmp['moyenne']) &&  intval($moyenne_tmp['moyenne'])>= 0) { 
                                        $var_coef =$var_coef + floatval($valuemat['coeficient_tmp']); 
                                        $var_moyen_coef=$var_moyen_coef + floatval($valuemat['coeficient_tmp'])* $moyenne_tmp['moyenne'];
                                    }
                                    elseif (20>=intval($moyenne_tmp['moyenne']) &&  intval($moyenne_tmp['moyenne'])>= 0) {
                                        $var_coef =$var_coef + floatval($valuemat['credit_tmp']); 
                                        $var_moyen_coef=$var_moyen_coef + floatval($valuemat['credit_tmp'])* $moyenne_tmp['moyenne'];
                                    }
                                    else { }
                                }
                            }
                        ...
                        //if ( $value['id_eleve_eleve']==89) { var_dump($get_grp_MatRepartie_WithMP_By); }
                        //var_dump('get_grp_MatRepartie_WithMP_By',$get_grp_MatRepartie_WithMP_By);
                        //if ( $value['id_eleve_eleve']==89) { var_dump('get_grp_MatRepartie_WithMP_By',$get_grp_MatRepartie_WithMP_By); }

                        if ($get_grp_MatRepartie_WithMP_By!=0) {
                            foreach ($get_grp_MatRepartie_WithMP_By as $keyMP => $valueMP) {
                                if ( intval($valueMP['id_annee_partie'])== $part_anneid ) {
                                    $moyenne_tmp = Admin::get_elevmoy_by($id_eleve,intval($valueMP['matiere_id_tmp']),$tabdatas['id_groupe'],$part_anneid);
                                    //if ( $value['id_eleve_eleve']==89) { var_dump('moyenne_tmp=',$moyenne_tmp ); }
                                    //var_dump($moyenne_tmp);
                                    //var_dump('valueMP',$valueMP);
                                    //var_dump('moyenne_tmp',$moyenne_tmp);
                                    if ($moyenne_tmp['moyenne']==null ) {
                                        $moyenne_tmp['moyenne']=0;
                                    }
                                    ...
                                    if ($valueMP['coeficient_tmp'] != '0' && 20>=intval($moyenne_tmp['moyenne']) &&  intval($moyenne_tmp['moyenne'])>= 0 ) { 
                                        $var_coef =$var_coef + floatval($valueMP['coeficient_tmp']); 
                                        $var_moyen_coef=$var_moyen_coef + floatval($valueMP['coeficient_tmp'])* $moyenne_tmp['moyenne'];
                                    }
                                    elseif (20>=intval($moyenne_tmp['moyenne']) &&  intval($moyenne_tmp['moyenne'])>= 0 ) {
                                        $var_coef =$var_coef + floatval($valueMP['credit_tmp']); 
                                        $var_moyen_coef=$var_moyen_coef + floatval($valueMP['credit_tmp'])* $moyenne_tmp['moyenne'];
                                    }
                                    else{ }

                                    $tb_mat_MP[$keyMP]=$valueMP;
                                    $tb_mat_MP[$keyMP]['moyenne']=floatval($moyenne_tmp['moyenne']);
                                    //if ( $value['id_eleve_eleve']==89) { var_dump('moyenne_tmp=',$moyenne_tmp );var_dump('var_coef=',$var_coef ); }
                                }
                            }
                        }  
                   ...
                   // var_dump('tb_moy_all',$tb_moy_all);exit;
                        ...
                        if ($get_grp_MatRepartie_WithMP_By!=0) {
                            ...
                            foreach ($get_grp_MatRepartie_WithMP_By as $key => $value) {
                                ...
                                if ( intval($value['id_annee_partie'])== $part_anneid ) {
                                    ...
                                    if ($moyenne_tmp['moyenne'] > 20 || 0>$moyenne_tmp['moyenne']) {
                                        $moyenne_tmp['moyenne']= -10;
                                    }
                                    if ($value['coeficient_tmp'] != '0' && $value['coeficient_tmp'] !=0  && $moyenne_tmp['moyenne'] >= 0 && 20>=$moyenne_tmp['moyenne']) { 
                                        $var_coef =$var_coef + floatval($value['coeficient_tmp']); 
                                        $var_moyen_coef=$var_moyen_coef + floatval($value['coeficient_tmp'])* $moyenne_tmp['moyenne'];
                                    }
                                    elseif($moyenne_tmp['moyenne'] >= 0 && 20>=$moyenne_tmp['moyenne']) { 
                                        $var_coef =$var_coef + floatval($value['credit_tmp']); 
                                        $var_moyen_coef=$var_moyen_coef + floatval($value['credit_tmp'])* $moyenne_tmp['moyenne'];
                                    }
