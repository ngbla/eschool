...
class Chat_realtime {
	...
	function user_login($user, $avatar){
		...
		$data = array();
		$sql=$this->dbh->prepare("SELECT name FROM message_admin_users WHERE name=?");
		$sql->execute(array($name));
		if($sql->rowCount() == 0){
			$upd=$this->dbh->prepare("INSERT INTO message_admin_users (name,avatar,login,status) VALUES (?,?,NOW(),?)");
			$upd->execute(array($name, $avatar, 'online'));
		}else{
			$upd=$this->dbh->prepare("UPDATE message_admin_users SET avatar=?, login=NOW(), status=? WHERE name=?");
	...
	function get_message($tipe, $ke, $user){
		...
		if($tipe == 'rooms'){
			if($ke == 'all'){
				$sql=$this->dbh->prepare("SELECT * FROM messages_admin WHERE tipe=? order by date DESC");
				$sql->execute(array($tipe));
			}else{
				$sql=$this->dbh->prepare("SELECT * FROM messages_admin WHERE ke=? order by date DESC");
			...
			if($ke == 'all'){
				$sql=$this->dbh->prepare("SELECT * FROM messages_admin WHERE (name = :id1 AND tipe= :id2) OR (ke = :id1 AND tipe = :id2) order by date DESC");
				...
				while($r = $sql->fetch()){
					...
					if(!in_array($name, $tmp)){
						array_push($tmp, $name);
						$get=$this->dbh->prepare("SELECT status FROM message_admin_users WHERE name=?");
				...
				$sql=$this->dbh->prepare("SELECT * FROM messages_admin WHERE (name = :id1 AND ke= :id2) OR (name = :id2 AND ke = :id1) order by date DESC");
	...
	function get_user($user){
		if(isset($user)){
			$sqlm=$this->dbh->prepare("SELECT name FROM message_admin_users WHERE name=?");
			$sqlm->execute(array($user));
			if($sqlm->rowCount() > 0){
				$upd=$this->dbh->prepare("UPDATE message_admin_users SET login=NOW() WHERE name=?");
				$upd->execute(array($user));
			}
		}
		$data = array();
		$sql=$this->dbh->prepare("SELECT * FROM message_admin_users");
		$sql->execute();
		while($r = $sql->fetch()){
			$data["all"][] = array(
				'name' => $r['name'],
		...
		$data["chat"] = $this->get_message("message_admin_users", "all", $user);
		return $data;
	}
	
	function send_message($name, $ke, $message, $image, $date, $avatar, $tipe){		
		$data = array();
		$sql=$this->dbh->prepare("INSERT INTO messages_admin (name,ke,avatar,message,image,tipe,date) VALUES (?,?,?,?,?,?,?)");
	...
	function user_logout($name){
		$data = array();
		$user = $this->dbh->prepare("UPDATE message_admin_users SET status=? WHERE name=?");
