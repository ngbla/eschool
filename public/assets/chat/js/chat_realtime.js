...
ajax("POST", "data=cek", res => {
    ...
    if (a.status == 'success') {
        ...
        document.getElementById("heading-online").innerHTML = "mathematique";
...
$(document).ready(function(){
    ...
    if ( typeof(sessionStorage.getItem("username")) != undefined && typeof(sessionStorage.getItem("avatar")) != undefined) {
        loginFunction();
      }
...
function loginFunction() {
    ...
    //const i = document.getElementById("username").value;
        ...
        const i = sessionStorage.getItem("username");
    ...
    if (i != '' && avatar != '') {
        ajax("POST", `data=login&name=${i}&avatar=${avatar}`, res => {
            ...
            if (a.status == 'success') {
                ...
                document.getElementById("ref").innerHTML = "<div class='alert alert-danger'>Username incorrect.</div>";
            }
        });
    } else {
        alert('Nom d\'utilisateur incorrect ! connectez vous! ')
    }
}
...
var chat_realtime = (j, k, l, m, n, imageDir, domain) => {
    ...
    userMysql(({all, chat}) => {
        ...
        chatMysql('mathematique', 'Public', a => {
            messages = a;
            no = 0;
    ...
    function messageHTML(a, bottom) {
        ...
        if (a.name == m) {
            ...
            if (uKe == "Public") {
                ...
                b += `<span class="message-username" style="color:#${col} !important">${a.name}</span>`;
            }
            b += `      <div class="message-text">${image != '' ? `<a title="Zoom" href="${imageDir}/${image}" class="placeholder"><img class="imageDir" src="${imageDir}/${image}"/></a>` : ''}${urltag(htmlEntities(a.message))}</div>`;
            b += `      <span class="message-time pull-right">${timeToWords(a.date)}</span>`;
            b += '	</div>';
    ...
    $('body').on('click', '.message-usernameDESACTIVER', function() {
        messages = [];
        const a = $(this).html();
