...
$(document).ready(function() {
    ...
    if (typeof(localStorage.getItem("admin_name")) != undefined && typeof(localStorage.getItem("admin_img") != undefined)) {
        ...
        var admin_img = localStorage.getItem("admin_img");
        console.log(admin_name, admin_img);
        $('#username').val(admin_name);
        $('#avatar').val(admin_img);
    }
...
ajax("POST", "data=cek", res => {
        ...
        //chat_realtime = (j, k, l, m, n, imageDir, domain)
...
var chat_realtime = (j, k, l, m, n, imageDir, domain) => {
    ...
    userMysql(({ all, chat }) => {
        console.log("userMysql", all, chat);
        allUser = all;
        chatUser = chat;
        allUser.forEach(a => {
            if (a.name != m) {
